...
import com.bur.blinote.database.NotesDatabase
import com.bur.blinote.entities.Notes
import com.bur.blinote.util.NoteBottomSheetFragment
...
class CreateNoteFragment : BaseFragment(),EasyPermissions.PermissionCallbacks,EasyPermissions.RationaleCallbacks{
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnOk.setOnClickListener {
            if (etWebLink.text.toString().trim().isNotEmpty()){
                ...
                Toast.makeText(requireContext(),"Ввод Url-адрес является обязательным",Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun saveNote(){
        ...
        if (etNoteTitle.text.isNullOrEmpty()){
            Toast.makeText(context,"Ввод заголовка является обязательным",Toast.LENGTH_SHORT).show()
        }
        else if (etNoteSubTitle.text.isNullOrEmpty()){

            Toast.makeText(context,"Ввод подзаголовка является обязательным",Toast.LENGTH_SHORT).show()
        }

        else if (etNoteDesc.text.isNullOrEmpty()){

            Toast.makeText(context,"Ввод описания является обязательным",Toast.LENGTH_SHORT).show()
        }
    ...
    private fun checkWebUrl(){
        if (Patterns.WEB_URL.matcher(etWebLink.text.toString()).matches()){
            ...
            Toast.makeText(requireContext(),"Url-адрес недопустим",Toast.LENGTH_SHORT).show()
        }
    }
